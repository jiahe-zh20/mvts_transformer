    ...
    """
        ...
        Attributes:
            ...
            max_seq_len: maximum sequence (time series) length. If None, script argument `max_seq_len` will be used.
                (Moreover, script argument overrides this attribute)
        """
    def __init__(self, mode, data_length=150):
        if mode == 'classification':
            x = pd.read_csv(f"raw_data/data_{data_length}.csv", header=None)
            y = pd.read_csv(f"raw_data/label_{data_length}.csv", header=None)
        else:
            x = pd.read_csv(f"raw_data/raw_data_{data_length}.csv", header=None)
            y = pd.read_csv(f"raw_data/raw_label_{data_length}.csv", header=None)
        x = x.to_numpy().reshape((-1, 9, data_length)).transpose(0, 2, 1).reshape((-1, 9))
        y = y.to_numpy()
        y = y[:, -1]
        ind = list(range(int(x.shape[0] / data_length)))
        ind = [id for id in ind for i in range(data_length)]
        self.all_df = pd.DataFrame(x, columns=['u'+format(i+1) for i in range(9)], dtype=np.float32)
        self.all_df.index = ind
        self.feature_df = self.all_df
        self.feature_names = self.feature_df.columns
        self.all_IDs = list(range(int(x.shape[0] / data_length)))
        self.labels_df = pd.DataFrame(y, dtype=np.int8)
        self.max_seq_len = data_length
