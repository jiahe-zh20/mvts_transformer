...
def main(config):
    ...
    my_data = data_class(config['task'])
    ...
    # Split dataset
    test_data = my_data
    test_indices = []  # will be converted to empty list in `split_dataset`, if also test_set_ratio == 0
    val_data = my_data
    if config['task'] == 'classification':
        val_data = data_class('imputation')
    ...
    #     # config['load_model'] = 'ex_log/epoch20/_2023-09-18_11-52-56_SN7/checkpoints/model_best.pth'  # 250
    #     config['load_model'] = 'ex_log/epoch20/_2023-09-18_18-55-03_fKk/checkpoints/model_best.pth'
